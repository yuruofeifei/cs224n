    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in range(len(sentence)):
            ...
            for j in xrange(-window_size, window_size + 1):
                if j + i < 0:
                    feature += start
                elif j + i >= len(sentence):
                    feature += end
                else:
                    feature += sentence[i + j]
            windowed_data.append((feature, labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(
            xavier_initializer((self.config.n_window_features*self.config.embed_size, self.config.hidden_size)),
            name="W")
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]), name="b1")
        U = tf.Variable(
            xavier_initializer((self.config.hidden_size, self.config.n_classes)),
            name="U")
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred), keep_dims=True)
        ### END YOUR CODE
        return loss
