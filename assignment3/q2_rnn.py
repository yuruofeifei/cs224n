...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) >= max_length:
            ret.append((sentence[0: max_length], labels[0: max_length], [True] * max_length))
        else:
            pad = max_length - len(sentence)
            mask = [True] * len(sentence)
            ret.append((sentence + ([zero_vector] * pad),
                       labels + ([zero_label] * pad),
                       mask + ([False] * pad)))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(
            tf.nn.embedding_lookup(self.pretrained_embeddings, self.input_placeholder),
    ...
    def add_prediction_op(self):
        ...
        U = tf.Variable(
            xavier_initializer((Config.hidden_size, Config.n_classes)),
            name="U")
        b2 = tf.Variable(tf.zeros([Config.n_classes]), name="b2")
        h_t = tf.constant(0, tf.float32, [1, Config.n_features * Config.embed_size], name="h")
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                preds.append(tf.expand_dims(y_t, 0))
                ### END YOUR CODE

        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
        #preds = tf.concat(0, preds)
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=logits), keep_dims=True)
        ### END YOUR CODE
        return loss
